...
- 静态类型语言。
- 与[Go语言](https://github.com/golang/go)完全兼容。
- 脚本化的风格，以及比Go更易于阅读的数据科学代码。
...
我们会保持Go+的简单性。这就是为什么我们称之为Go+，而不是Go++。
...
将支持所有Go的特性（包括部分支持`cgo`，[见下文](#bytecode-vs-go-code)）。

Go+可以导入所有Go软件包（即使是这些软件包使用cgo）。
...
然后在Go软件包（14-Using-goplus-in-Go/gomain）中使用它：
...
基于GopherJS的Go+ Playground（目前仅在v0.7.x中可用）：
...
当我们使用`gop go`或`gop install`命令时，它会生成Go代码，将Go+软件包隐藏到Go软件包中。

当我们使用`gop run`命令时，它不会调用`go run`命令。而是生成要执行的字节码（在v1.0.x中，go run也使用Go代码生成模式）。

在字节码模式下，Go+不支持`cgo`。不过，在Go代码生成模式下，Go+完全支持cgo。
...
我们将有理数作为原生Go+类型引入。我们使用后缀`r`来表示有理数字面量。例如，（1r << 200）表示值等于2<sup>200</sup>的 big int 型整数。4/5r 表示有理常数 4/5。
...
注意：对于udt.Gop_Enum（回调）的范围，无法使用 break/continue或return语句。
...
我们在Go+中重新发明了错误处理规范。我们称之为`ErrWrap表达式`：
...
这些`ErrWrap表达式`是如何工作的？有关更多信息，请参阅[错误处理](https://github.com/goplus/gop/wiki/Error-Handling)。
...
在Go+中，我们引入了一个名为`自动属性`的概念。这是一个`get属性`，但会自动实现。如果我们有一个名为`Bar()`的方法，那么我们将同时有一个名为`bar`的`get属性`。


### Unix shebang

您现在可以使用 Go+ 程序作为shell脚本。例如：
...
将支持所有Go特性（包括部分支持cgo）。在字节码模式下，Go+不支持`cgo`。然而，在Go代码生成模式下，Go+完全支持`cgo`。
