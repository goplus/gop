...
import (
	...
	"github.com/goplus/gop/gopdoc/lazyregexp"
...
func (r *reader) readPackage(pkg *ast.Package, mode Mode) {
	...
	var overloadFunc = make(map[string]string)
	...
	for _, f := range pkg.Files {
							...
							if strings.Contains(name.Name, "Gopo_") {
								for _, v := range vs.Values {
									if bas, ok := v.(*ast.BasicLit); ok {
										fmt.Println(bas.Value)
										overloadFuncs := strings.Split(bas.Value, ",")
										for i2 := range overloadFuncs {
											overloadFunc[strings.ReplaceAll(overloadFuncs[i2], "\"", "")] = strings.TrimPrefix(name.Name, "Gopo_")
										}
									}
								}
							}
		...
		for _, decl := range f.Decls {
			if d, ok := decl.(*ast.FuncDecl); ok {
				if k, ok := overloadFunc[d.Name.Name]; ok {
					d.Name.Name = fmt.Sprintf("%s!%s", k, uuid.NewString())
				}
				r.readFunc(d)
			}
		}
