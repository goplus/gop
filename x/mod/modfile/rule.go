...
// Package modfile implements a parser and formatter for gop.mod files.
//
// The gop.mod syntax is described in
// https://golang.org/cmd/go/#hdr-The_go_mod_file.
//
// The Parse and ParseLax functions both parse a gop.mod file and return an
// abstract syntax tree. ParseLax ignores unknown statements and may be used to
// parse gop.mod files that may have been developed with newer versions of Go.
//
// The File struct returned by Parse and ParseLax represent an abstract
// gop.mod file. File has several methods like AddNewRequire and DropReplace
...
func (f *File) add(errs *ErrorList, block *LineBlock, line *Line, verb string, args []string, fix VersionFixer, strict bool) {
	...
	if !strict {
		switch verb {
		case "go", "gop", "classfile", "register", "module", "retract", "require":
			// want these even for dependency gop.mods
		default:
			return
		}
	}
	...
	case "classfile":
		if len(args) <= 2 {
			errorf("usage: %s [classfileExt...] pkgPathOfClassFile", verb)
			return
		}
	...
	case "retract":
		...
		if err != nil {
			if strict {
				...
				// gop.mod parser, even in lax mode.
				return
			}
		}
...
// meaning it is in gop.mod only for its effect on indirect dependencies,
...
func IsDirectoryPath(ns string) bool {
	// Because gop.mod files can move from one system to another,
...
func parseExt(s *string) (string, error) {
	...
	if len(t) < 2 {
		return "", &InvalidExtError{
			Ext: *s,
			Err: fmt.Errorf("invalid ext format"),
		}
	}
	if t[0] != '.' {
		return "", &InvalidExtError{
...
func (e *InvalidExtError) Error() string {
	return fmt.Sprintf("ext %q invalid: %s", e.Ext, e.Err)
}
