...
func TestParsePunctuation(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.desc, func(t *testing.T) {
			f, err := parse("gop.mod", []byte(test.src))
			if err != nil {
...
func TestGoVersion(t *testing.T) {
	...
	t.Run("Strict", func(t *testing.T) {
		for _, test := range tests {
			t.Run(test.desc, func(t *testing.T) {
				if _, err := Parse("gop.mod", []byte(test.input), nil); err == nil && !test.ok {
	...
	t.Run("Lax", func(t *testing.T) {
		for _, test := range tests {
			t.Run(test.desc, func(t *testing.T) {
				if _, err := Parse("gop.mod", []byte(test.input), nil); err == nil && !(test.ok || test.laxOK) {
					t.Error("unexpected success")
				} else if err != nil && test.ok {
					t.Errorf("unexpected error: %v", err)
				}
...
`,
		...
		t.Run(test.desc, func(t *testing.T) {
			f, err := ParseLax("gop.mod", []byte(test.input), nil)
			if err != nil {
				t.Fatal(err)
			}
