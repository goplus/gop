...
var builtinCastOps = [...]func(i Instr, p *Context){
	...
	(int(UnsafePointer) << bitsKind) | int(Uintptr): execUnsafePointerFromUintptr,
}
...
func execUnsafePointerFromUintptr(i Instr, p *Context) {
	n := len(p.data)
	p.data[n-1] = unsafe.Pointer(p.data[n-1].(uintptr))
}
