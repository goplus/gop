...
const (
	...
	generatedHeader = "// Code generated by go+; DO NOT EDIT.\n"
)
...
func genGoSingleFile(file string, conf *Config, flags GenFlags) (err error) {
	...
	f, err := os.Create(autogen)
	if err != nil {
		return errors.NewWith(err, `Create(autogen)`, -2, "os.Create", autogen)
	}
	defer f.Close()
	fmt.Fprint(f, generatedHeader)

	if err = out.WriteTo(f); err != nil {
		return errors.NewWith(err, `out.WriteTo(autogen)`, -2, "(*gox.Package).WriteTo", out, autogen)
	}
...
func genGoIn(dir string, conf *Config, genTestPkg bool, flags GenFlags, gen ...*bool) (err error) {
	...
	if err != nil {
		return errors.NewWith(err, `Create(file)`, -2, "os.Create", file)
	}
	defer f.Close()
	fmt.Fprint(f, generatedHeader)
	...
	err = out.WriteTo(f)
	if err != nil {
		return errors.NewWith(err, `out.WriteTo(file)`, -2, "(*gox.Package).WriteTo", out, file)
	}
...
func GenGoFiles(autogen string, files []string, conf *Config) (result []string, err error) {
	...
	f, err := os.Create(autogen)
	if err != nil {
		err = errors.NewWith(err, `Create(autogen)`, -2, "os.Create", autogen)
		return
	}
	defer f.Close()
	fmt.Fprint(f, generatedHeader)
	...
	err = out.WriteTo(f)
	if err != nil {
		err = errors.NewWith(err, `out.WriteTo(autogen)`, -2, "(*gox.Package).WriteTo", out, autogen)
	}
	return
}
